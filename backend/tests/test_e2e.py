def test_e2e_flow(client):
    ...
    # register admin
    r = client.post('/auth/register', json={'email':'admin@test.local','password':'secret'})
    assert r.status_code == 200
    ...
    client.post('/auth/register', json={'email':'analyst@test.local','password':'secret'})
    client.post('/users/assign-role', headers=HADM, json={'email':'analyst@test.local','role':'analyst'})
    tok = client.post('/auth/login', json={'email':'analyst@test.local','password':'secret'}).json()['access_token']
